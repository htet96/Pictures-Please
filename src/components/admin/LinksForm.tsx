...
export function LinksForm({ initialLinks, linksPublic: initialLinksPublic }: Props) {
  const [links, setLinks] = useState<LinkEntry[]>(
    initialLinks.length > 0 ? initialLinks : []
  );
  const [linksPublic, setLinksPublic] = useState(initialLinksPublic);
    ...
    try {
      const [linksRes, settingsRes] = await Promise.all([
        ...
        fetch("/api/settings", {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ linksPublic }),
        }),
      ]);
      if (!linksRes.ok || !settingsRes.ok) throw new Error();
      setLinks(valid);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between gap-4 pb-4 border-b border-border">
        <div>
          <Label className="text-sm font-medium">Visible to everyone</Label>
          <p className="text-xs text-muted-foreground mt-0.5">
            When off, the kebab menu is only shown to admins.
          </p>
        </div>
        <Switch
          checked={linksPublic}
          onCheckedChange={setLinksPublic}
        />
      </div>
